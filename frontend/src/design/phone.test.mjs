 ...
 * in the right order — "091 98765 43210" is a real way to write a number, and
...
test('the country code is dropped however it was pasted', () => {
  for (const raw of ['+91 98765 43210', '+919876543210', '91 9876543210', '(+91)-98765-43210']) {
    assert.equal(normalizePhone(raw), '9876543210', raw);
  }
});

test('a trunk zero is dropped, before or after the country code', () => {
  assert.equal(normalizePhone('09876543210'), '9876543210');
  assert.equal(normalizePhone('091-98765-43210'), '9876543210');
  assert.equal(normalizePhone('0 +91 9876543210'), '9876543210');
});
...
test('an eleventh digit is ignored rather than pushing the first one off', () => {
  ...
  assert.equal(normalizePhone('98765432101'), '9876543210');
});

test('anything that is not a digit is discarded', () => {
  assert.equal(normalizePhone('98a76b54c3210'), '9876543210');
...
test('formatPhone leaves a number it did not store alone', () => {
  ...
  assert.equal(formatPhone('+91 9876543210'), '+91 9876543210');
});
...
test('normalizing is idempotent, so a stored value round-trips unchanged', () => {
  const once = normalizePhone('+91 98765 43210');
